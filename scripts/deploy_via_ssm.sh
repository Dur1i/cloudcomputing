...
systemctl disable tc-social.service 2>/dev/null || true
systemctl stop tcblog.service 2>/dev/null || true

mkdir -p /opt/tcblog /etc/tcblog
install -o ubuntu -g ubuntu -m 640 "$BUNDLE_DIR/websocial.jar" /opt/tcblog/websocial.jar
install -o ubuntu -g ubuntu -m 750 "$BUNDLE_DIR/scripts/run_application.sh" /opt/tcblog/run_application.sh
install -o root -g root -m 644 "$BUNDLE_DIR/deploy/tcblog.service" /etc/systemd/system/tcblog.service
...
journalctl -u tcblog.service -n 150 --no-pager
exit 1
